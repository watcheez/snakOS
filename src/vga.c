...
#define FONT_PADDING_TOP 3
#define FONT_PADDING_RIGHT 1
...
{
    outportb(PALETTE_MASK, 0xff);      /* Should be 0xff normally */
    outportb(PALETTE_WRITE, 0x00);
    for (uint8_t i = 0; i < 255; ++i) {
        outportb(PALETTE_DATA, (uint8_t) (((i >> 5) & 0x7) * (256 / 8) / 4));  /* Dividing by four because of */
        outportb(PALETTE_DATA, (uint8_t) (((i >> 2) & 0x7) * (256 / 8) / 4));
        outportb(PALETTE_DATA, (uint8_t) ((i & 0x3) * (256 / 4) / 4));
    }
...
extern void drawIcon(uint32_t x, uint32_t y, struct icon icon)
{
    ...
    for (j = 0; j < icon.height; j++) {
        for (i = 0; i < icon.width; i++) {
            drawPixel(x + i, y + j, icon.data[j * icon.width + i]);
        }
    }
}
...
extern void writeChar(uint32_t x, uint32_t y, unsigned char c) 
{
    ...
    size_t l = c * FONT_HEIGHT;
    size_t rx = x * (FONT_WIDTH + FONT_PADDING_RIGHT);
    size_t ry = y * (FONT_HEIGHT + FONT_PADDING_TOP);

    if (x >= 0 && y >= 0 && y * SCREEN_WIDTH + x < SCREEN_HEIGHT * SCREEN_WIDTH) {
